...
import { bridalGowns } from '@/data/mock/bridal';
...
export default function BridalGalleryPage() {
    ...
    const filteredGowns = filter === 'all'
        ? bridalGowns
        : bridalGowns.filter(gown => gown.silhouette.toLowerCase() === filter);

    const silhouettes = ['all', ...Array.from(new Set(bridalGowns.map(g => g.silhouette.toLowerCase())))];
                            ...
                            <div className="relative aspect-[3/4] overflow-hidden bg-slate-100">
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/70 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                                    <div className="flex gap-2">
                                        <Button size="sm" className="flex-1 bg-white text-slate-900 hover:bg-slate-100">
                                            Book Fitting
                                        </Button>

...
export default function BridalManager() {
      ...
      const url =
        activeTab === "gowns"
          ...
          : activeTab === "services"
            ...
            : "/api/admin/catalog/bridal/accessories";
      const res = await fetch(url);
      const data = await res.json();
      if (activeTab === "gowns") setGowns(data.gowns || []);
      else if (activeTab === "services") setServices(data.services || []);
      else setAccessories(data.accessories || []);
    } catch (error) {
  ...
  const handleOpenModal = (item?: Gown | Service | Accessory) => {
    if (activeTab === "gowns") {
      ...
      if (item) {
        setEditingItem(accessory);
        setAccessoryForm({
          name: accessory.name,
          category: accessory.category,
          price_rent: accessory.price_rent?.toString() || "",
          price_buy: accessory.price_buy?.toString() || "",
    ...
    try {
      ...
      const payload =
        activeTab === "gowns"
          ? {
              ...gownForm,
              price_rent: gownForm.price_rent
                ? parseFloat(gownForm.price_rent)
                : null,
              price_buy: gownForm.price_buy
                ? parseFloat(gownForm.price_buy)
                : null,
            }
          : activeTab === "services"
            ? {
                ...serviceForm,
                price_start: serviceForm.price_start
                  ? parseFloat(serviceForm.price_start)
                  : null,
              }
            : {
                ...accessoryForm,
                price_rent: accessoryForm.price_rent
                  ? parseFloat(accessoryForm.price_rent)
                  : null,
                price_buy: accessoryForm.price_buy
                  ? parseFloat(accessoryForm.price_buy)
                  : null,
              };
      ...
      fetchData();
      addToast(
        `${activeTab === "gowns" ? "Gown" : activeTab === "services" ? "Service" : "Accessory"} saved successfully`,
        "success"
      );
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-1 rounded-xl border border-slate-200 w-fit">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === "accessories" ? "bg-pink-100 text-pink-700 shadow-sm" : "text-slate-500 hover:text-primary"}`}
        >
          Accessories
        </button>
      </div>
              ...
              <div className="aspect-square bg-slate-100 relative overflow-hidden">
                ...
                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="p-2 bg-white rounded-lg shadow-sm text-slate-600 hover:text-pink-600"
                  >
                    <Edit2 size={16} />
                  </button>
                  <button
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <div className="flex gap-1">
                    <button
                      ...
                      className="p-1.5 hover:bg-slate-100 rounded text-slate-400 hover:text-pink-600"
                    >
                      <Edit2 size={14} />
                    </button>
                    <button
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      type="number"
                      className="w-full px-4 py-2 border border-slate-200 rounded-lg"
                      value={accessoryForm.price_rent}
                      onChange={(e) =>
                        setAccessoryForm({
                          ...accessoryForm,
                          price_rent: e.target.value,
                        })
                      }
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <input
                      type="number"
                      className="w-full px-4 py-2 border border-slate-200 rounded-lg"
                      value={accessoryForm.price_buy}
                      onChange={(e) =>
                        setAccessoryForm({
                          ...accessoryForm,
                          price_buy: e.target.value,
                        })
                      }
                    />
                  </div>
                  <div className="col-span-2 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {accessoryForm.images.map((url, index) => (
                        <ImageUpload
                          key={index}
                          bucket="bridal"
                          value={url}
                          onChange={(newUrl) => {
                            const newImages = [...accessoryForm.images];
                            if (newUrl) {
                        ...
                        <ImageUpload
                          ...
                          onChange={(newUrl: string) => {
                            if (newUrl) {
                              setAccessoryForm({
                                ...accessoryForm,
                                images: [...accessoryForm.images, newUrl],
                              });
                            }
                          }}
                        />
                      ...
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            ...
                            if (url && accessoryForm.images.length < 4) {
                              setAccessoryForm({
                                ...accessoryForm,
                                images: [...accessoryForm.images, url],
                              });
                              input.value = "";
                            }
                          }
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2 space-y-1">
                    ...
                    <input
                      ...
                      value={gownForm.name}
                      onChange={(e) =>
                        setGownForm({ ...gownForm, name: e.target.value })
                      }
                    />
                  </div>
                  <div className="space-y-1 text-xs">
                    ...
                    <input
                      type="text"
                      className="w-full px-4 py-2 border border-slate-200 rounded-lg"
                      value={gownForm.designer}
                      onChange={(e) =>
                        setGownForm({ ...gownForm, designer: e.target.value })
                      }
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setGownForm({ ...gownForm, silhouette: e.target.value })
                      }
                    >
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      type="number"
                      className="w-full px-4 py-2 border border-slate-200 rounded-lg"
                      value={gownForm.price_rent}
                      onChange={(e) =>
                        setGownForm({ ...gownForm, price_rent: e.target.value })
                      }
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <input
                      type="number"
                      className="w-full px-4 py-2 border border-slate-200 rounded-lg"
                      value={gownForm.price_buy}
                      onChange={(e) =>
                        setGownForm({ ...gownForm, price_buy: e.target.value })
                      }
                    />
                  </div>
                  <div className="col-span-2 space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      id="is_new"
                      checked={gownForm.is_new}
                      onChange={(e) =>
                        setGownForm({ ...gownForm, is_new: e.target.checked })
                      }
                    />
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button
                  disabled={submitting}
                  type="submit"
                  className="flex-1 px-4 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700 transition-colors flex items-center justify-center gap-2 font-medium"
                >

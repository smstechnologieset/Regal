...
export default function AttireManager() {
    ...
    const [highlightFilter, setHighlightFilter] = useState<'all' | 'new' | 'bestseller'>('all');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (stockCount > 0 && !formData.in_stock) {
            addToast('Products with stock count > 0 must be marked as "In Stock"', 'error');
            return;
        }
        ...
        if (originalPrice !== null && originalPrice <= price) {
            addToast('Original Price must be greater than current Price to create a Sale', 'error');
            return;
        }
    ...
    return (
        <div className="space-y-6">
            ...
            {(stockFilter !== 'all' || saleFilter !== 'all' || highlightFilter !== 'all' || categoryFilter !== '') && (
                <div className="flex items-center gap-2 flex-wrap pb-2">
                    <span className="text-xs text-slate-400">Active Admin Filters:</span>
                    {categoryFilter && (
                        <FilterBadge label={`Category: ${categories.find(c => c.id === categoryFilter)?.name}`} onClear={() => setCategoryFilter('')} />
                    )}
                    {stockFilter !== 'all' && (
                        <FilterBadge label={`Stock: ${stockFilter.replace('_', ' ')}`} onClear={() => setStockFilter('all')} />
                    )}
                    {saleFilter !== 'all' && (
                        <FilterBadge label={`Sale: ${saleFilter === 'on_sale' ? 'Yes' : 'No'}`} onClear={() => setSaleFilter('all')} />
                    )}
                    {highlightFilter !== 'all' && (
                            ...
                            <div className="aspect-square bg-slate-100 relative overflow-hidden">
                                ...
                                <div className="absolute top-2 left-2 flex flex-col gap-1">
                                    {product.badges?.includes('new') && (
                                        <span className="bg-emerald-500 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">NEW</span>
                                    )}
                                    {product.badges?.includes('bestseller') && (
                                        <span className="bg-amber-500 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">BEST SELLER</span>
                                    )}
                                    {product.original_price && product.original_price > product.price && (
                                        <span className="bg-rose-500 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">SALE</span>
                                    )}
                                </div>
                                {!product.in_stock && (
                    ...
                    <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1">
                                    <label className="text-sm font-medium text-slate-700">Original Price ($) <span className="text-[10px] text-slate-400 ml-1">(Optional - for Sale)</span></label>
                                    <input
                                        type="number"
                                        step="0.01"
                                        className="w-full px-4 py-2 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500"
                                ...
                                <div className="col-span-2 space-y-3">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                ...
                                                setFormData({
                                                    ...formData,
                                                    colors: [...formData.colors, { name: colorName.trim(), hex: colorHex }]
                                                });
                                                setColorName('');
                                            }}
                                    ...
                                    <p className="text-[10px] text-slate-400">Add colors one by one with their name and pick a hex value.</p>
                                </div>
                            </div>
